...
from deepCR.dataset import dataset, dataset_inpaint, dataset_inpaint_obj
from deepCR.unet_ssli import UNet

__all__ = ['train_mask', 'train_inpaint', 'train_inpaint_obj']
...
class train_inpaint_obj():

    def __init__(self, train_data_dir, train_image_files, train_CRmask_files, train_clean_files, train_OBJmask_files,
                    validation_data_dir, validation_image_files, validation_CRmask_files, validation_clean_files, validation_OBJmask_files,
                    train_ignore_files=None, validation_ignore_files=None, 
                    train_sky_val_list=None, validation_sky_val_list=None,
                    aug_sky=[0, 0],
                    name='model', 
        ...
        train_image_files : a list of str
            A list of image file names for training
        train_CRmask_files : a list of str
            A list of CR mask file names for training
        train_clean_files : a list of str
            A list of clean image file names for training
        train_OBJmask_files : a list of str
            A list of clean image object mask file names for training
        validation_data_dir : str
            Directory where all validation data are saved
        validation_image_files : a list of str
            A list of image file names for validation
        validation_CRmask_files : a list of str
            A list of CR mask file names for validation
        validation_clean_files : a list of str
            A list of clean image file names for validation
        validation_OBJmask_files : a list of str
            A list of clean image object mask file names for validation
        train_ignore_files : a list of str, optional
            A list of file names for mask of bad pixel, saturation, etc.
        validation_ignore_files : a list of str, optional
            A list of file names for mask of bad pixel, saturation, etc.
        ...
        # check images and masks
        for data_dir, image_files, CRmask_files, clean_files, OBJmask_files in ((train_data_dir, train_image_files, train_CRmask_files, train_clean_files, train_OBJmask_files), 
                                                (validation_data_dir, validation_image_files, validation_CRmask_files, validation_clean_files, validation_OBJmask_files)):
            ## number of images and masks should match
            assert ((len(image_files) == len(CRmask_files)) and (len(image_files) == len(clean_files)) 
                    and (len(image_files) == len(OBJmask_files)))
            ## shape of image and mask should match
            ima_tmp = np.load(os.path.join(data_dir, image_files[0]))
            CRmask_tmp = np.load(os.path.join(data_dir, CRmask_files[0]))
            clean_tmp = np.load(os.path.join(data_dir, clean_files[0]))
            OBJmask_tmp = np.load(os.path.join(data_dir, OBJmask_files[0]))
            del image_files, CRmask_files, clean_files, OBJmask_files
            assert ((ima_tmp.shape == CRmask_tmp.shape) and (ima_tmp.shape == clean_tmp.shape)
                    and (ima_tmp.shape == OBJmask_tmp.shape))
            del CRmask_tmp, clean_tmp, OBJmask_tmp
            ## image should be square
            assert (ima_tmp.shape[0] == ima_tmp.shape[1])
            del ima_tmp
        ...
        # create torch dataset
        data_train = dataset_inpaint_obj(train_data_dir, train_image_files, train_CRmask_files, 
                    train_clean_files, train_OBJmask_files,
                    train_ignore_files, train_sky_val_list, aug_sky, gpu, scale)
        data_val = dataset_inpaint_obj(validation_data_dir, validation_image_files, validation_CRmask_files, 
                    validation_clean_files, validation_OBJmask_files,
                    validation_ignore_files, validation_sky_val_list, aug_sky, gpu, scale)
        del train_data_dir, train_image_files, train_CRmask_files, 
        del train_clean_files, train_OBJmask_files, 
        del train_ignore_files, train_sky_val_list, aug_sky
        del validation_data_dir, validation_image_files, validation_CRmask_files, 
        del validation_clean_files, validation_OBJmask_files
        del validation_ignore_files, validation_sky_val_list
    ...
    def validate_mask(self):
        ...
        with torch.no_grad():
            for dat in self.ValLoader:
                ...
                # get data
                if self.ignore_map:
                    images, CRmasks, clean_images, OBJmasks, ignores = dat
                else:
                    images, CRmasks, clean_images, OBJmasks = dat
                    ignores = None
                del dat

                # predict the inpaint
                pdt_images = self.network(torch.cat((images*(1-CRmasks), CRmasks), dim=1))
                del images
                pdt_noCRs = pdt_images * CRmasks * OBJmasks
                del pdt_images
                true_noCRs = clean_images * CRmasks * OBJmasks
                del clean_images, CRmasks, OBJmasks
