...
class deepCR():
    ...
    def clean_(self, img0, threshold=0.5, inpaint=True, binary=True):
        ...
        if inpaint and (self.inpaintNet is not None):
            ...
            if binary:
                return binary_mask[pad_x:, pad_y:], inpainted[pad_x:, pad_y:] * self.scale
            else:
                mask = mask.detach().cpu().view(shape[0], shape[1]).numpy()
                return mask[pad_x:, pad_y:], inpainted[pad_x:, pad_y:] * self.scale
